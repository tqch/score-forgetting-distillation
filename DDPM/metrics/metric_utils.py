...
class MetricOptions:
    def __init__(self, G=None, init_t=None, init_a=None, init_b=None, init_sigma=None,
                 G_kwargs={}, dataset_kwargs={}, data_stat=None, num_gpus=1, rank=0,
                 local_rank=0, device=None, progress=None, cache=True):
...
def compute_feature_stats_for_dataset(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=64,
    ...
    if data_loader_kwargs is None:
        num_workers = min(4, int(os.environ.get("NUM_WORKERS", "4")))
        prefetch_factor = min(2, num_workers) if num_workers else None
        data_loader_kwargs = dict(pin_memory=True, num_workers=num_workers, prefetch_factor=prefetch_factor)
...
def compute_feature_stats_for_generator(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=128,
                                        batch_gen=None, jit=False, **stats_kwargs):
    if batch_gen is None:
        batch_gen = min(batch_size, 128)
    ...
    def run_generator(xt, c):
        t = torch.empty([xt.size(0), ], device=opts.device).fill_(init_t)
        et = G(xt, t, c, cond_scale=0.0, cond_drop_prob=0.0, mode="test")
        x0 = xt / init_a - et * init_sigma
        img = (x0 * 127.5 + 128).clamp(0, 255).to(torch.uint8)
        return img

    # JIT.
    if jit:
        z = init_b * torch.zeros([batch_gen, G.img_channels, G.img_resolution, G.img_resolution],
                                     device=opts.device)

        c = torch.zeros([batch_gen, G.c_dim], device=opts.device)
        run_generator = torch.jit.trace(run_generator, [z, c, init_b], check_trace=False)
    ...
    while not stats.is_full():
        ...
        for _ in range(batch_size // batch_gen):
            z = init_b * torch.randn(
                [batch_gen, G.img_channels, G.img_resolution, G.img_resolution], device=opts.device)
            c = [dataset.get_label(np.random.randint(len(dataset))).argmax() for _ in range(batch_gen)]
            c = torch.as_tensor(c).pin_memory().to(opts.device)
