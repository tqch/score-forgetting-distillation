...
def get_dataset(args, config, sampler_fn=None):
    """
    ...
    shuffle = True
    sampler = None
    dataloader_kwargs = dict(pin_memory=True, prefetch_factor=2)
    if sampler_fn is not None:
        sampler = sampler_fn(len(dataset))
        shuffle = False
...
def get_forget_dataset(args, config, label_to_drop, sampler_fn=None):
    """
    Returns vanilla CIFAR10/STL10 dataset.
    """
    if config.data.random_flip is False:
    ...
    for _dataset in datasets:
        try:
            targets.append(torch.as_tensor(_dataset.targets))
        except AttributeError:
            targets.append(torch.as_tensor(_dataset.labels))
    targets = torch.cat(targets, dim=0)
    data_remain = torch.utils.data.Subset(dataset, torch.where(targets != label_to_drop)[0])
    data_forget = torch.utils.data.Subset(dataset, torch.where(targets == label_to_drop)[0])
    ...
    shuffle = True
    world_size = int(os.environ.get("WORLD_SIZE", "1"))
    remain_sampler = forget_sampler = None
    if sampler_fn is not None:
        shuffle = False
        remain_sampler = sampler_fn(len(data_remain))
        if len(data_forget) > 0:
            forget_sampler = sampler_fn(len(data_forget))
    batch_size = config.training.batch_size // world_size
